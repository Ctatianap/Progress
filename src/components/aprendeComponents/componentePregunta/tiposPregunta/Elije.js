...
const Elije = ({ pregunta, proximaPregunta, id }) => {
  ...
  const toggleRespuesta = (event) => {
    ...
    if (respuestaUser) {
      //Se busca la respuesta seleccionnada anterior y se le cambia la clase
  ...
  return (
    <div className="container-fluid d-flex flex-column pregunta-container">
      <h4>{titulo}</h4>
      <img
        className="img-cerdito"
        src={require(`../../../../assets/img/${descripcion}`)}
        alt="cerdito"
      />
